...
# ── Frontend-aligned shape (what the UI actually renders) ──
class ChallengeListItemResponse(BaseModel):
    """Challenge shape that matches the RektoFun frontend expectations."""
    id: int
    title: str = ""
    description: str = ""
    ...
    mode: str = ""
    initial_bet: int = 0
    min_accept_bet: int = 0
    max_accept_bet: int = 0
    min_bet: int = 0
    bet_unit: int = 0
    total_pool: int = 0
    status: str = "open"
...
class FrontendChallengesResponse(BaseModel):
    """Response wrapper matching frontend getChallenges() expectations."""
